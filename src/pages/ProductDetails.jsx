...
const ProductDetails = () => {
  const { product, addToCart, navigate } = UseAppContext();
  const { id } = useParams();

  // Find the single product using the ID from the URL
  ...
  // Set the first product image as thumbnail
  useEffect(() => {
  ...
  if (!products) {
    return (
      <div className="py-20 text-center">
        ...
        <button
          onClick={() => navigate("/")}
          className="mt-5 px-6 py-3 bg-indigo-500 text-white rounded cursor-pointer"
        >
          Go Home
        </button>
      </div>
